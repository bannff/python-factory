 ...
 */
export function isPictograph(name) {
    const s = name.trim();
    if (!s || s.length > 8)
        return false;
 ...
 * token as text.
 */
export function ViewIcon({ name, className, textClassName }) {
    if (!name || !name.trim())
        return null;
    const Icon = resolveIcon(name);
    if (Icon) {
        return _jsx(Icon, { "aria-hidden": "true", className: cn("h-4 w-4 shrink-0", className) });
    }
    if (isPictograph(name)) {
        return (_jsx("span", { "aria-hidden": "true", className: cn("shrink-0 leading-none", textClassName), children: name.trim() }));
    }
    return null;
}
