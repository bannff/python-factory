"""Integration tests for the `props.children` lift in ``ui_paint_canvas``.

Carved out of ``test_paint_canvas.py`` to keep that file under 200 LOC
(bd:python-factory-3hkqx round 3). These verify the producer-side lift
end-to-end: from agent-emitted nested ``props.children`` to canonical
flat siblings on the wire.
"""
...
def test_props_children_lifted_to_parent_refs():
    ...
    components = result["_a2ui_canvas"]["payload"]["components"]
...
def test_props_children_without_id_surfaces_validation_error():
    """Lift descends into props.children without enforcing id presence;
    validate_a2ui catches the missing id and the agent self-corrects."""
    result = _make_tool()(
        target="live",
        payload={"components": [{
            "id": "card", "type": "Card",
