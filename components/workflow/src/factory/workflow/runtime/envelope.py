...
def parse_envelope(value: dict[str, Any] | None) -> Envelope:
    ...
    # Typed MCP DTO dumps represent an omitted transport timestamp as null.
    if normalized.get("timestamp") is None:
        normalized.pop("timestamp", None)
    return Envelope.model_validate(normalized)


class FrozenEnvelope(Envelope):
    """Persisted initiation context with no field reassignment."""
